...
const REQUIRED_ENGINE_VERSION = "1.2.2";
...
export class StatusCenterHost {
  ...
  private async collect(): Promise<StatusCenterData> {
    ...
    const capabilities = capabilityStatus(binary, hub, mcp);
    return {
  ...
  private async binaryStatus(): Promise<BinaryStatus> {
    ...
    try {
      ...
      const modern = current ? compareVersion(current, REQUIRED_ENGINE_VERSION) >= 0 : false;
      return {
        found: true,
        path: this.binary,
        version,
        level: modern ? "ok" : "warn",
        message: modern
          ? "Strata CLI is available."
          : `Strata CLI is available, but ${REQUIRED_ENGINE_VERSION} or newer is recommended for this extension branch.`,
      };
    } catch (error) {
      return {
        found: true,
...
function suggestedFixes(
  ...
  } else if (!binaryMeetsMinimum(binary)) {
    fixes.push({
      title: "Upgrade Strata CLI",
      detail: `Install strata ${REQUIRED_ENGINE_VERSION} or newer to enable the current output and browsing contracts.`,
      level: "warn",
      action: { op: "open-settings" },
    });
  }
...
function binaryMeetsMinimum(binary: BinaryStatus): boolean {
  if (!binary.found || !binary.version) return false;
  const parsed = parseVersion(binary.version);
