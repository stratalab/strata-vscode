...
import { binaryMeetsMinimum, capabilityStatus, REQUIRED_ENGINE_VERSION, versionMeetsMinimum } from "../status/capabilities";
...
export class StatusCenterHost {
  ...
  private async binaryStatus(): Promise<BinaryStatus> {
    ...
    try {
      ...
      const modern = versionMeetsMinimum(version);
      return {
        found: true,
        path: this.binary,
        version,
